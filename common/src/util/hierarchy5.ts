...
export const createHierarchy = <T, D extends Diff<T, any>>(
	...
	if (!differentiators?.children?.length) {
		console.log("no children - early return []");
		return {
			debug: differentiators.debug,
			groupName: "TODO final", // differentiators.getGroupName(currentItems[0]),
	...
	return {
		...
		children: [
			...
			...(!differentiators.useSingleChildDifferForAllGroups ?
				...
				: children.length !== 1 ? ([new Error("children length must be 1")])
